...
describe("IdentityFederationService (Business Validation)", () => {
  let instance: IdentityFederationService;
  ...
  beforeAll(() => {
    ...
    try {
      instance = new IdentityFederationService(
        prisma as any,
        eventEmitter as any,
        genericMock as any,
        genericMock as any,
  ...
  it("should execute business logic for configureSaml", async () => {
    if (!instance.configureSaml) return;
    try {
      const result = await instance.configureSaml(
