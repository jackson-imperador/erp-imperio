...
describe("SpedService (Business Validation)", () => {
  let instance: SpedService;
  ...
  it("should execute business logic for generateSped", async () => {
    if (!instance.generateSped) return;
    try {
      const result = await instance.generateSped(
