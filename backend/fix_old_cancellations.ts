...
async function fixOldCancellations() {
  console.log("Iniciando varredura de vendas antigas canceladas que não tiveram rollback...");
  ...
  for (const sale of cancelledSales) {
    ...
    // 1. Corrigir Contas a Receber e Transações Financeiras (Visão Geral)
    for (const rec of sale.receivables) {
      ...
      for (const t of transactions) {
        console.log(`[Venda #${sale.orderNumber}] Corrigindo Transação Financeira (Visão Geral) ID: ${t.id}`);
    ...
    // Se houver um SALE, mas não houver um WITHDRAWAL (Estorno) para essa venda, significa que ficou pendente.
    ...
    if (hasSale && !hasWithdrawal) {
      console.log(`[Venda #${sale.orderNumber}] Dinheiro órfão detectado no PDV! Gerando estorno...`);
