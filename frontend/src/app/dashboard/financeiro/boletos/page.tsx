...
export default function BoletosPage() {
  ...
  const boletoTransactions = transactions.filter(t => t.paymentMethod === 'BOLETO');
    ...
    },
    {
      ...
      render: (row: FinancialTransaction) => (
        <Link href={`/dashboard/financeiro/${row.id}`}>
          <Button variant="outline" size="sm">Gerenciar Boleto</Button>
        </Link>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Boletos Bancários</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Gestão de carteira de boletos registrados
          </p>
        </div>
        <Button size="sm"><FileText className="w-4 h-4 mr-2" />Novo Boleto Avulso</Button>
      </div>

      <div className="bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <GenericDataTable data={boletoTransactions} columns={columns} isLoading={isLoading} />
      </div>
    </div>
  );
}
