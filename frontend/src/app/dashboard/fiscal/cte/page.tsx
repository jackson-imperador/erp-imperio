...
export default function CtePage() {
  const [filters, setFilters] = useState<FiscalFilters>({ type: 'CTE' });
  ...
  const columns = [
    ...
    { key: 'customerName', header: 'Tomador do Frete', render: (r: FiscalDocument) => r.customerName || '-' },
    { key: 'totalAmount', header: 'Valor Frete', render: (r: FiscalDocument) => `R$ ${(r.totalAmount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Conhecimento de Transporte (CT-e)</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Gestão de documentos de transporte de cargas
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/fiscal/cte/novo">
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />Emitir CT-e</Button>
          </Link>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={() => setFilters({ type: 'CTE' })}>Limpar Filtros</Button>
          </div>
        </div>
      )}
