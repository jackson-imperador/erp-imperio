...
export default function MdfePage() {
  const [filters, setFilters] = useState<FiscalFilters>({ type: 'MDFE' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Manifesto Eletrônico (MDF-e)</h1>
        ...
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/fiscal/mdfe/novo">
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />Emitir MDF-e</Button>
          </Link>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={() => setFilters({ type: 'MDFE' })}>Limpar Filtros</Button>
          </div>
        </div>
      )}
