...
export default function NfcePage() {
  const [filters, setFilters] = useState<FiscalFilters>({ type: 'NFCE' });
  ...
  const columns = [
    ...
    { key: 'customerName', header: 'Consumidor', render: (r: FiscalDocument) => r.customerName || 'Consumidor Não Identificado' },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">NFC-e (Consumidor)</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Gestão de notas modelo 65 (Venda no balcão)
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => exportToCsv('nfce.csv', documents)}>
            <FileDown className="w-4 h-4 mr-2" />Exportar
          </Button>
          <Link href="/dashboard/fiscal/nfce/nova">
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />PDV (Nova NFC-e)</Button>
          </Link>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={() => setFilters({ type: 'NFCE' })}>Limpar Filtros</Button>
          </div>
        </div>
      )}
