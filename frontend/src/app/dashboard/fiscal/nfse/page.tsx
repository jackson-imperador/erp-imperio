...
export default function NfsePage() {
  const [filters, setFilters] = useState<FiscalFilters>({ type: 'NFSE' });
  const [showFilters, setShowFilters] = useState(false);
  const { data: documents = [], isLoading } = useFiscalDocuments(filters);

  const columns = [
    { key: 'number', header: 'Número', render: (r: FiscalDocument) => `${r.number}` },
    { key: 'customerName', header: 'Tomador do Serviço', render: (r: FiscalDocument) => r.customerName || '-' },
    { key: 'totalAmount', header: 'Valor Serviço', render: (r: FiscalDocument) => `R$ ${(r.totalAmount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` },
    { key: 'status', header: 'Status Prefeitura', render: (r: FiscalDocument) => <FiscalStatusBadge status={r.status} /> },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">NFS-e (Serviços)</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Gestão de notas fiscais de serviço eletrônica municipal
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => exportToCsv('nfse.csv', documents)}>
            <FileDown className="w-4 h-4 mr-2" />Exportar
          </Button>
          <Link href="/dashboard/fiscal/nfse/nova">
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />Emitir NFS-e</Button>
          </Link>
        </div>
      </div>
      ...
      {showFilters && (
        <div className="bg-white dark:bg-zinc-900 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <div className="flex items-end">
            <Button variant="outline" size="sm" onClick={() => setFilters({ type: 'NFSE' })}>Limpar Filtros</Button>
          </div>
        </div>
      )}
